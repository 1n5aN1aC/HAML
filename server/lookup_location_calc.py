...
fact about it — CQ and ITU zone numbers. The module is split so a new
derivation only has to add its own query + entry point:
...
def _lookup_region(table, value_column, lat, lon):
    """Value of the first region in `table` whose polygon covers the point.
    ...
    Three things the query depends on:
      ...
      - `ORDER BY z.id` decides overlaps deterministically. CQ and ITU zones
        overlap slightly and leave gaps (they cover ~96% and ~94.5% of the
        globe), so a point matches two regions or none. None is a correct
        answer and comes back as None.
    """
    conn = _conn()
    if conn is None:
        return None
    try:
        rows = conn.execute(
            f"SELECT p.part_id, p.geom, z.{value_column} "
        ...
        return None
    for part_id, geom, value in rows:
...
def derive_zones(lat, lon):
    ...
    return {
        "cq_zone": _lookup_region("cq_zones", "zone", lat_f, lon_f),
        "itu_zone": _lookup_region("itu_zones", "zone", lat_f, lon_f),
    }
